...
from camayoc.constants import VAULT_PASSWORD, VAULT_PASSWORD_INPUT
...
def auth_add(options, inputs=None, exitstatus=0, vault_password=VAULT_PASSWORD):
    ...
    if "name" in options:
        assert rho_auth_add.expect('Auth "{}" was added'.format(options["name"])) == 0
