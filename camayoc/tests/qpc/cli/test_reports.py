...
"""Expected fields for each report entry in the report list response."""


@pytest.mark.runs_scan
def test_report_list_contains_scan_report(
    data_provider, scans, isolated_filesystem, qpc_server_config
):
    """Ensure ``qpc report list`` contains the report generated by a scan.

    :id: 7d3e9f1a-5b8c-4a62-c0d1-2e4f6a8b0c93
    :description: Run a scan, then verify that ``qpc report list`` includes
        the report generated by that scan with correct metadata.
    :steps:
        1) Run a scan and obtain its report_id.
        2) Run ``qpc report list``.
        3) Verify the report_id from the scan is present in the list.
        4) Verify the report entry has correct scan association.
    :expectedresults: The report from the completed scan appears in the
        report list with the correct scan_id.
    """
    finished_scan = scan_with_source_type(("network", "satellite", "vcenter"), data_provider, scans)
    expected_report_id = finished_scan.report_id
    assert expected_report_id, "No report id was returned from scan"

    output = report_list()
    report_data = json.loads(output)

    report_ids = [entry["id"] for entry in report_data["results"]]
    assert expected_report_id in report_ids, (
        f"Report {expected_report_id} from completed scan not found in report list. "
        f"Available report IDs: {report_ids}"
    )

    matching_report = next(
        entry for entry in report_data["results"] if entry["id"] == expected_report_id
    )
    actual_fields = set(matching_report.keys())
    expected_fields = set(REPORT_LIST_FIELDS)
...
@pytest.mark.runs_scan
def test_report_show_matches_list_entry(
    data_provider, scans, isolated_filesystem, qpc_server_config
):
    """Ensure ``qpc report show`` output matches the corresponding list entry.

    :id: 9a1c3e5f-7b2d-4f84-d0e6-4c8a2f6b0d15
    :description: Compare the output from ``qpc report show`` with the
        corresponding entry from ``qpc report list`` to ensure consistency.
    :steps:
        1) Run a scan to ensure at least one report exists.
        2) Run ``qpc report list`` and pick a random report entry.
        3) Run ``qpc report show --report <report-id>``.
        4) Compare the show output with the list entry.
    :expectedresults: The report show output matches the corresponding
        entry in the report list for all common fields.
    """
    scan_with_source_type(("network", "satellite", "vcenter"), data_provider, scans)

    list_output = report_list()
    list_data = json.loads(list_output)

    list_entry = random.choice(list_data["results"])
    report_id = list_entry["id"]

    show_output = report_show({"report": report_id})
    show_entry = json.loads(show_output)

    for field in REPORT_LIST_FIELDS:
        assert show_entry[field] == list_entry[field], (
            f"Mismatch for field '{field}': show={show_entry[field]!r}, list={list_entry[field]!r}"
        )


def test_report_show_nonexistent_id(isolated_filesystem, qpc_server_config):
    """Ensure ``qpc report show`` fails gracefully for a nonexistent report ID.

    :id: 6e2b4a8f-3d1c-4f95-b7e0-9c5a1d3f8e62
    :description: Run ``qpc report show`` with an ID that does not exist on the
        server and verify the command exits with a non-zero status.
    :steps:
        1) Run ``qpc report show --report <nonexistent-id>``.
        2) Verify the command exits with status code 1.
    :expectedresults: The command fails with exit code 1 indicating the
        report does not exist.
    """
    report_show({"report": sys.maxsize}, exitstatus=1)
