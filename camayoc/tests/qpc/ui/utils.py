...
def row_xpath(row_name):
    ...
    """
    return (
        f'//div[contains(@class, "list-view-pf-top-align")'
...
def delete_xpath(row_name):
    ...
    """
    return (
        f'//div[contains(@class, "list-view-pf-top-align") and '
...
def check_auth_type(credential_name, auth_type):
    ...
    Locator(
        xpath=(
            f'//span[text()="{auth_type}" and ancestor::node()[2]'
...
def create_credential(view, options):
    ...
    try:
        ...
        add_credential_dropdown.item_select(options["source_type"] + " Credential")
    except NoSuchElementException:
    ...
    assert modal.save_button.browser.get_attribute("disabled", modal.save_button)
    fill_credential_info(view, options)

    assert not modal.save_button.browser.get_attribute("disabled", modal.save_button)
...
def delete_credential(view, names):
    ...
    DeleteModalView(view, locator=Locator(css=".modal-content")).delete_button.click()
    # Wait for the deletion animations to complete,
    # and verify that the rows are gone.
    wait_for_animation(1)
    for name in names:
        with pytest.raises(NoSuchElementException):
            view.wait_for_element(locator=Locator(xpath=row_xpath(name)), timeout=1)
...
def edit_credential(view, original_name, options):
    ...
    GenericLocatorWidget(view, locator=Locator(xpath=edit_xpath(original_name))).click()
    modal = CredentialModalView(view, locator=Locator(css=".modal-content"))
    wait_for_animation(1)
    fill_credential_info(view, options)
    # Hack to deal with the fact that the GET refresh isn't
...
def create_source(view, credential_name, source_type, source_name, addresses):
    ...
    wait_for_animation()
    GenericLocatorWidget(modal, locator=Locator(xpath=radio_xpath(radio_label))).click()
    wait_for_animation(1)
...
def delete_source(view, source_name):
    ...
    GenericLocatorWidget(view, locator=Locator(xpath=delete_xpath(source_name))).click()
    # mitigate database lock issue quipucords/quipucords/issues/1275
    wait_for_animation()
